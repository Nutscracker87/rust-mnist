...
/// Sentinel digit value: no MNIST label is 99, so passing this disables weight visualisation for that run.
const DIGIT_NO_VISUALISATION: usize = 99;
...
/// Runs the test set, counts correct predictions, and optionally prints weight visualisations
/// for a chosen digit (only on the final epoch). Use `DIGIT_NO_VISUALISATION` to skip printing.
fn test_nn_and_print_results(
    nn: &Network,
    epoch: usize,
    test_data_set: &[Sample],
    epoch_secs: f64,
